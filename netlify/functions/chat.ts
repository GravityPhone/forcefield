...
- knock_logs(id, person_id -> persons.id, household_id -> addresses.id, canvasser_id -> profiles.id, occurred_at, outcome, notes, squad_id, squad_name, turf_id, turf_name) — outcome is one of 'signed','didnt_sign','maybe','not_home','skip','hostile'; the app LABELS two of them differently from the stored value — didnt_sign is "Not Interested", maybe is "Come back another time" — so use that wording in prose and chart labels while querying the stored value; join addresses on knock_logs.household_id = addresses.id for the address. squad_*/turf_* are stamped at insert time (the crew the canvasser was with that local day; the door's TOP-LEVEL turf) with the _name columns snapshotted — for historical "which squad/turf did these knocks belong to" questions GROUP BY the _name columns; they survive squad deletion and turf re-cuts where the _id joins go stale
