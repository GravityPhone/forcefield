...
- leaderboard_settings / activity_feed_settings — single-row config tables (WHERE id = true): the leaderboard's ranking metric, and the team activity feed's event toggles + milestone steps (per-person/squad/whole-team doors & signatures). Only relevant if asked how the boards or the live feed are configured
