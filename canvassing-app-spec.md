...
- **2026-07-07 — turf cutter gestures + UI pass:** the geolocate action was renamed **"Place pins"** and moved out of the layers strip to its own button on all three maps (the shading toggle is now labeled "Shade"). The sweep bar gained an **Add/Erase mode toggle** — Erase runs the identical gestures in reverse (two-tap walk removes that stretch, splitting a segment around the hole; double-tap drops a street; hold removes one door). **Holding a pin (~450 ms)** takes exactly that single door in or out of the draft. Every draft-changing gesture snapshots first, powering an **Undo** button (up to 25 steps); "Clear" became **"Clear all"** and is itself undoable. **Turf names are optional** — saving blank auto-names the cut "&lt;assignee&gt;'s turf" (or "Turf N" when unassigned). The draft tray was redesigned from stacked editor boxes into **compact pills** (street · range · door-count badge, ✕ to drop) with a single inline editor that opens for whichever pill is tapped.
