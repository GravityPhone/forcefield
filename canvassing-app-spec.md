...
## 0. Current Status (as of 2026-07-20)
...
- **Reka UI behavior primitives**, themed 100% by the existing CSS-variable schemes: `src/components/ui/BottomSheet.vue` (Dialog — used by /admin/roles editor, Squads/Chat composers, and the nav More sheet) and `src/components/ui/AppSelect.vue` (Select — replaced every native `<select>`).
...
- **Same day, evening — turf works across days (dispatch model,** `20260707120000_turf_dispatch.sql`**):** squads stay one-day crews, but turf is now explicitly the durable layer — `turfs.squad_id` is a DISPATCH the campaign manager re-points at each day's squads from `/turf` ("cut once, send different crews on different days"). Turf still pointed at a past day's squad is hidden from the Squad page, Hunt, and sub-cutter scoping (today-only filters client-side; the DB sub-cut guards stay date-agnostic because `squad_date` is a client-local day) and instead wears an amber "Not out today — last crew was X (Jul 6)" flag on `/turf`, making that list the morning dispatch board. Re-dispatching a turf server-side DISSOLVES its sub-turfs (trigger — the per-member "<name>'s doors" splits are the old crew's internal division; doors return to the parent) and APPENDS to the new `turf_assignments` history table (trigger-written, squad names snapshotted since day-squad rows are deletable; unassignment logs an all-null row). `/turf` shows each turf's last few crews ("Crews: Jul 5 Terrarium 2 · Jul 6 Kool krew"), and the AI chat knows the new table. Verified live: triggers, backfill, dissolution, no-op-update guard, and the new PostgREST embeds all tested against the production DB.
...
- **2026-07-12 — people pages:** everyone got an **"About me" profile** (`/profile`: bio / why-I'm-canvassing / fun-fact fields + an optional phone number in team-private `member_phones` — RLS-gated Call buttons appear beside teammates with a saved number on the roster, member pages, Squad cards, and the chat member menu) and a **team Roster** (`/roster`, admins pick a team to browse) whose rows open `/member/:id` — intro + their last 100 knocks. Talk mode also floods person bubbles / the address banner with the door's latest outcome color.
- **Same day — Roles rename + My knocks:** with the Roster now owning "browse people," the old Users screen was renamed **Roles** (`/admin/roles`, old `/admin/users` redirects) and framed as pure role/team/today's-squad placement. New **"My knocks"** page (`/history`) for canvassers, squad leaders, and CMs: your own knock history ("did I already hit that street?") — last 500 knocks grouped by day, street/name search, outcome filter chips, and tapping a visit reopens that door in Talk mode for follow-ups.
- **2026-07-13/14 — the maps broke under their own data, then got fast:** three-part repair after mass geocoding (~9.9k pinned doors) and the simulated month of knock history (~15k `household_latest_knock` rows) outgrew hidden limits. (1) **PostgREST row cap:** Supabase caps every REST response at `max_rows` = 1000 regardless of client `.limit()` — Hunt was drawing the first 1000 pins and coloring the first 1000 doors, silently. All whole-set queries (Hunt pins/statuses/summaries/knocked-today, Turf cutter addresses/statuses, Squad doors/knocks) now page through `fetchAllRows()` (`src/lib/supabase.ts`), which fetches 1000-row pages in concurrent batches of 4. (2) **Vector basemap died:** the same day, maps created with our cloud map ID stopped painting their basemap everywhere — map initializes, pins/overlays draw, tiles stay blank beige; no auth failure, all RPCs 200; reproduced on both weekly and quarterly API channels, isolated via labeled A/B/C test maps on a visible tab. Escape: every map now passes `renderingType: 'RASTER'` (`MAP_RENDERING_TYPE` in `src/lib/googleMaps.ts`) — raster paints fine with the same map ID and AdvancedMarkers only need a mapId, not vector. (3) **Hunt pin virtualization:** with 10k doors, building an AdvancedMarker each made phones crawl — Hunt now creates markers only for doors inside the padded viewport (rebuilt on map `idle`, capped at 2000 nearest-to-center) and below zoom 15 shows only clickable density dots (grid-bucketed, same look as the cluster bubbles; tap zooms into that patch). Door DATA all still loads for turf shading/search/colors; MarkerClusterer removed from Hunt (Squad/Turf keep theirs).

- **2026-07-14, later — turf/squad history, turf for everyone, demo role switcher:** (1) `knock_logs` now stamps context at insert (`20260714120000`): `squad_id`/`squad_name` (the squad the canvasser was crewing with that local day) and `turf_id`/`turf_name` (the door's top-level turf at knock time — sub-turfs resolve to their parent, since per-member splits dissolve nightly). Names are snapshots, so history survives squad wipes and turf re-cuts; the ~19.8k existing knocks were backfilled (99.6% matched a squad — the sim's recurring "<leader>'s crew" names aggregate cleanly across days). (2) **Analytics** grew **Turfs** and **Squads** tabs (signatures / sign-rate / turf coverage; per-squad days-out tables) plus a turf filter scoping every tab. (3) The **Scout map's turf shading returned** — removed that morning, rebuilt as a "Turf" layer toggle visible to EVERY role (own turf still shades stronger), with an "All turf" zoom chip so even an unassigned canvasser can see how the ground is cut. (4) **Demo role switcher:** the header's role badge is now a button (non-admins) opening a sheet that says it's demo-only and lets the visitor move between Canvasser / Squad Leader / Campaign Manager — `demo_set_own_role()` + a transaction-local escape hatch in `guard_profile_privileges`; admin unreachable both directions, direct self-promotion still blocked (verified live).

- **2026-07-18 — squad page for everyone, status-colored squad pins, Talk↔Scout flow, turf-shading controls:** (1) `/squad` now serves ANY member's own squad — campaign managers included (a CM in a crew gets the full squad page, not just its chat); `?squad=<id>` picks between multiple squads, the Squads roster page sorts a CM's own crews to the top with an "Open" button, and creating a squad lands on the squad page. (2) Squad-map door pins switched from knocked-vs-todo green/blue to the SAME `doorStatusOutcome` status colors as Scout (statuses chunk-fetched from `household_latest_knock` by door id; a live knock refetches just that door's row). (3) Talk gained a **Previous** button beside Next (then = the street walked the opposite way; reworked 07-20, below), and flipping to Scout auto-orients on the door Talk is working — street name filled into the search, door located and highlighted, once per Talk address so manual Scout browsing survives tab flips. (4) The Scout chip strip slimmed to one "Zoom to my turf" chip, and the import-era "Place pins" buttons were removed from all three maps — cut-time geocoding (cutting a turf geocodes its doors) plus Talk/Scout locate-on-demand are the only geocoding paths now. (5) Turf shading became a tri-state control: Scout has "My turf" / "All turf" radio-with-off buttons (pref shared with the cutter's single Shade button), and the Squad map got its own "Our turf" / "All turf" strip under a separate key defaulting to the crew's turf — its org-wide door download only happens the first time "All turf" is switched on.
- **2026-07-20 — walk navigation v2 + today's-knocker avatars:** (1) Talk's **"Up next" chips** preview the next three doors the walk pattern (direction / side / partly-signed pref) would visit — same skip rules as Next via the shared `findUpcomingOnStreet` — each chip a house number with the door's status color; tapping one jumps straight there. (2) **Previous now retraces the canvasser's own knock history** instead of walking the street backwards: distinct knocked doors newest-first (session knocks prepended live, the server's last 500 merged in on first use), position stateless (the current door's index in the path). A **"‹ Back" chip** at the head of the Up-next row does the same without needing an outcome logged, so repeated stepping works; the big Previous/Next pair still only appears after logging. (3) **Squad-map avatar pins:** doors a squad member knocked TODAY wear that member's animal avatar on the door pin (derived client-side from the page's existing knock fetch + live inserts; latest knock wins; initial-letter fallback; hidden during assign mode).
- **2026-07-20, later — team activity feed** (`/activity`, "Feed" tab for canvassers/leads/CMs): a today-only, org-wide live feed — every signature as it lands (and every knock, if enabled) plus **milestone moments**: personal door counts (every 5 by default), squad doors/signatures for the day (25/10), and whole-team "way to go"s (100 doors / 25 signatures), with a pulsing "Today so far: X doors · Y signatures" strip. Entirely computed client-side from `knock_logs` (squad attribution via the 07-14 stamps; realtime INSERT channel with enrich-by-id refetch). Doors count DISTINCT households and signatures DISTINCT signed persons — the squad-progress semantics, not the leaderboard's every-log count; "team" = everyone out today, deliberately not `team_id`-scoped. The campaign manager's knobs live in a new single-row `activity_feed_settings` table (`20260720130000`, leaderboard_settings pattern: everyone reads, `is_admin()` writes) with an "Activity Feed" card on `/admin` — event toggles are separate from milestone toggles, so a manager can mute the knock firehose and keep the hype. Verified live as a signed-in sim canvasser (embed query, settings read, canvasser write correctly blocked by RLS).

- **2026-07-23 — turf cutter rebuilt twice, landing street-first:** the morning rework moved all drafting client-side and onto one canvas (full address table in memory, instant gestures, VAN-style lasso, angular turf shading on every map, "honest segments" in the `set_turf_segments` RPC so a turf's stored ranges can never cover another turf's doors). The evening rework (user call: fewer buttons, whole streets) made STREETS the unit of work: the map draws one smoothed line per street, colored by ownership (gray unclaimed, saved turf's color, thick draft stroke); search a street → tap a match → the map zooms there; tap a street's line → every door on it joins the draft; tap a drafted street → trim mode, where just that street's doors appear as status-colored dots/number-pills and each tap drops or restores one house. Lasso, Undo, and the per-segment range/side pill editor remain; gone are the anchor two-tap sweep, hold-to-toggle, double-tap street toggle, dots/numbers pin toggle, global Both/Even/Odd row, and per-search-row "Entire street" buttons. Also fixed: door dots no longer drift during zoom animations (the canvas now tracks the live projection with a scale transform and repaints only when the zoom settles), and startup is map-first — the map appears immediately and flies to the user's own location zoomed in, street data streams in behind it, and knock statuses don't load until trim mode first opens (previously everything blocked on a multi-megabyte load behind "Loading doors…").

- **2026-07-24 — turf cutter goes search-first with a blank map:** the street lines from the day before (drawn by connecting door geocodes) didn't sit on the real roads, so the map now draws nothing street-shaped at all — it starts blank (basemap + optional turf shading), zoomed to the user. The flow: type a street name, tap a match to zoom there (its doors appear as dots), then an explicit "Add to turf" button on the located row takes every door; already-added rows show "In this turf ✓". Street-line tapping is gone entirely — map taps only matter in trim mode (open a street's pill, tap doors to drop/restore houses). The always-rendered turf list at the bottom was replaced by one dropdown: picking a turf zooms the map to it and opens a single compact management card (edit / delete / reassign / crew history). Later the same day: draft doors now stay painted as streets accumulate (the turf builds up visibly on the map), the located row's Add takes an editable house-number range (whole street by default — narrow it to take just a portion), and the lasso gained an Erase mode — an Add/Erase toggle appears while it's armed, and a red erase loop pulls the circled doors back out of the draft. Evening additions: a "☝ Streets" tool — arm it and tap a road right on the basemap (no dots needed) to take or drop that entire street, sharing the lasso's Add/Erase switch; the lasso now geocodes the streets it captures so the dots actually populate; and tapping any painted dot (no tool armed) pops a compact house-history bubble (last few knocks with outcome, who, and date). Follow-up fix: every door a draft's ranges cover now paints — including doors owned by another turf (they wear that turf's ring and stay out of the save), which was why a tapped street could add with no pins — drafted streets geocode all their unmapped doors instead of a capped 25, and opening a turf for editing geocodes the whole turf so it's fully pinned.

- **2026-07-24, later — the tap tool learns to read the map, and geocoding learns to say no:** field testing near Richwood exposed three linked bugs. (1) Tapping S Clinton St kept adding E Bomford St: the ☝ Streets tool resolved taps to the *nearest geocoded door*, and in a town where only the sim-turf streets ever got geocoded (231 doors, all bulk-written 07-23), the street under your finger often had zero mapped doors — its geocoded neighbor won every time, and once that wrong street was in the draft, further taps hit "already in this turf" and the tool read as dead. Taps now **reverse-geocode the tapped point** to a street *name* (matched to the voter file through a USPS-style normalizer — "South Clinton Street" ⇒ "S CLINTON ST", "OH-4" ⇒ "STATE ROUTE 4"), which works on streets with no coordinates at all; the nearest-door vote survives only as an offline fallback with a tap-again-to-confirm guard when nothing mapped is close. (2) Pins "popping up in random areas" were **poisoned cached geocodes**: Google returns a town-centroid APPROXIMATE result rather than admit it can't find an address, and the client cached whatever came back — verified live as 136 doors stacked on 9 exact coordinates (83 doors from 11 different Marysville streets shared the town centroid). `geocodeAndCache` now validates before caching (street-level result + route-name match + county bounds — otherwise cache nothing and stay retryable), and a new `scripts/clean-bad-geocodes.mjs` nulled the stacks and re-geocoded them under the same rules: 48 doors recovered to real positions, 88 now honestly unmapped. (3) The lasso "not working at all" in Richwood was the same coverage hole — it can only catch doors already painted as dots, so an empty loop now says exactly that; and on half-geocoded streets its runs no longer split around doors that *couldn't* be circled (unmapped house numbers between circled neighbors ride along instead of punching phantom holes).
...
**Live:** https://forcefield.ninja (custom domain since 2026-07-13; https://f0rcef1eld.netlify.app is the same Netlify site) — admin credentials live in the gitignored `KEYS-AND-ACCESS.md`, not in the repo.
